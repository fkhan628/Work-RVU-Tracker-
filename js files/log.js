...
function LogProc({ data, db, cptMap, categories, upd, setView, showUndo }) {
  ...
  var [clinicResults, setClinicResults] = useState(null);
  ...
  var scanClinic = function(file) {
    if (!file || scanning.current) return;
    getDecryptedApiKey(data.settings).then(function(apiKey) {
    if (!apiKey) { setScanError("No API key configured. Add one in Settings to enable scanning."); return; }
    scanning.current = true;
    var isPDF = file.type === "application/pdf" || (file.name && file.name.toLowerCase().endsWith(".pdf"));
    setScanStatus(isPDF ? "Reading PDF..." : "Reading schedule...");
    setScanError("");
    ...
    reader.onload = function(ev) {
      var base64 = ev.target.result.split(",")[1];
      var mediaType = isPDF ? "application/pdf" : (file.type || "image/jpeg");
      setScanStatus("Analyzing clinic schedule...");
      var contentBlock = isPDF
        ? { type: "document", source: { type: "base64", media_type: "application/pdf", data: base64 } }
        : { type: "image", source: { type: "base64", media_type: mediaType, data: base64 } };
      fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              contentBlock,
              { type: "text", text: "You are analyzing a clinic schedule (which may be a PDF document or image) for a surgical RVU tracking app. This shows patient appointments for a clinic day. The document may be multi-page - analyze ALL pages thoroughly.\n\nCRITICAL PRIVACY RULE: Do NOT include any Protected Health Information (PHI) in your response. Never return patient full names, MRNs, dates of birth, SSNs, addresses, phone numbers, diagnoses, or any other HIPAA identifiers. For patient field, return ONLY 2-letter initials (e.g. \"JD\"). For visit_type, use only generic descriptions (e.g. \"follow-up\", \"new consult\") - never include specific diagnoses or clinical details.\n\nExtract each patient encounter:\n1. patient: 2-letter initials ONLY derived from patient name. If name is not clear, use slot numbers like \"P1\", \"P2\".\n2. visit_type: GENERIC visit type only (new patient, follow-up, post-op, consult, procedure, etc.) - NO diagnoses or clinical details\n3. codes: array of likely CPT codes with description. Use these E&M guidelines:\n   - New patient office visits: 99202 (straightforward), 99203 (low), 99204 (moderate), 99205 (high)\n   - Established patient: 99211 (minimal/nurse), 99212 (straightforward), 99213 (low), 99214 (moderate), 99215 (high)\n   - Post-op visits within global period: no E&M code (0.00 wRVU) - mark as \"post-op global\"\n   - If a procedure is noted (biopsy, I&D, excision, wound care, etc.), include the procedure CPT code\n   - Consults: use appropriate new patient E&M level\n4. confidence: high/medium/low\n\nAlso extract:\n- date: the clinic date in YYYY-MM-DD format if visible\n- If the schedule shows appointment times, types, or reasons, use those to determine visit complexity\n\nRespond ONLY with valid JSON, no markdown backticks:\n{\"date\": \"YYYY-MM-DD\", \"encounters\": [{\"patient\": \"XX\", \"visit_type\": \"description\", \"codes\": [{\"code\": \"XXXXX\", \"description\": \"...\", \"confidence\": \"high\"}]}]}" }
            ]
          }]
        })
      }).then(function(res) { return res.json(); }).then(function(apiData) {
        var text = "";
        if (apiData.content) {
          apiData.content.forEach(function(item) { if (item.type === "text") text += item.text; });
        }
        text = text.replace(/```json|```/g, "").trim();
        try {
          var result = JSON.parse(text);
          ...
          if (result.encounters && result.encounters.length > 0) {
            var processed = result.encounters.map(function(enc, idx) {
              var resolvedCodes = (enc.codes || []).map(function(c) {
                var found = cptMap[c.code];
                return {
                  code: c.code,
                  description: found ? found.desc : c.description,
                  wRVU: found ? found.wRVU : 0,
              ...
              return {
                id: idx,
                patient: enc.patient || ("P" + (idx + 1)),
                visitType: enc.visit_type || "Office visit",
                codes: resolvedCodes,
                selected: true
              };
            });
            setClinicResults(processed);
            setScanStatus("Found " + processed.length + " encounter(s)");
            setTimeout(function() { setScanStatus(""); }, 2000);
          } else {
            setScanStatus("");
            setScanError("No encounters found in schedule");
          }
        } catch (e) {
          setScanStatus("");
          setScanError("Could not parse response. Try a clearer image or PDF.");
        }
        scanning.current = false;
      }).catch(function(err) {
        setScanStatus("");
    ...
    reader.readAsDataURL(file);
    }); // end getDecryptedApiKey for scanClinic
  };

  var toggleClinicEnc = function(idx) {
    setClinicResults(function(prev) {
      return prev.map(function(e, i) { return i === idx ? { ...e, selected: !e.selected } : e; });
    });
  };

  var saveClinicBatch = function() {
    if (!clinicResults) return;
    var newEntries = [];
    clinicResults.forEach(function(enc) {
      if (!enc.selected) return;
      enc.codes.forEach(function(c) {
        if (!c.valid) return;
        var info = cptMap[c.code];
        if (!info) return;
        newEntries.push({
          id: Date.now().toString() + "-clinic-" + enc.id + "-" + c.code,
          date: date,
          cptCode: c.code,
          ...
          baseRVU: info.wRVU,
          modifiers: [],
          adjustedRVU: info.wRVU,
          notes: enc.patient + " - " + enc.visitType,
          encounterId: enc.patient,
          isCall: isCall,
          imported: false
        });
      });
    ...
    if (newEntries.length > 0) {
      ...
      var totalBatchRVU = newEntries.reduce(function(s, e) { return s + e.adjustedRVU; }, 0);
      setScanStatus("Logged " + newEntries.length + " procedures (" + totalBatchRVU.toFixed(1) + " wRVUs)");
      setClinicResults(null);
      setTimeout(function() { setScanStatus(""); }, 3000);
    }
  };
  ...
  return (<div style={S.page}>
    ...
    {(function() { var hasKey = hasApiKey(data.settings); return (<div style={{ marginBottom: 12 }}>
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <input ref={clinicRef} type="file" accept="image/*,application/pdf" style={{ display: "none" }} onChange={function(e) { if (e.target.files && e.target.files[0]) scanClinic(e.target.files[0]); e.target.value = ""; }} />
        <button onClick={function() { if (!hasKey) { setScanError("No API key. Go to Settings to add one."); return; } if (hipaaAck) { imgRef.current && imgRef.current.click(); } else { pendingScanType.current = "opnote"; setShowHipaaWarn(true); } }} disabled={scanning.current} style={{ flex: 1, padding: "12px 10px", borderRadius: 12, border: hasKey ? "1px dashed #0ea5e9" : "1px dashed #334155", background: hasKey ? "rgba(14,165,233,0.05)" : "rgba(30,41,59,0.5)", color: hasKey ? "#0ea5e9" : "#475569", fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 2 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}><span style={{ fontSize: 18 }}>&#128247;</span> Scan Op Note</div>
          <div style={{ fontSize: 9, fontWeight: 400, opacity: 0.7 }}>Photo or PDF</div>
        </button>
        <button onClick={function() { if (!hasKey) { setScanError("No API key. Go to Settings to add one."); return; } if (hipaaAck) { clinicRef.current && clinicRef.current.click(); } else { pendingScanType.current = "clinic"; setShowHipaaWarn(true); } }} disabled={scanning.current} style={{ flex: 1, padding: "12px 10px", borderRadius: 12, border: hasKey ? "1px dashed #10b981" : "1px dashed #334155", background: hasKey ? "rgba(16,185,129,0.05)" : "rgba(30,41,59,0.5)", color: hasKey ? "#10b981" : "#475569", fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 2 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}><span style={{ fontSize: 18 }}>&#128197;</span> Clinic Schedule</div>
          <div style={{ fontSize: 9, fontWeight: 400, opacity: 0.7 }}>Photo or PDF</div>
        </button>
      </div>
      {!hasKey && <div style={{ marginTop: 6, fontSize: 11, color: "#475569", textAlign: "center" }}>Add an API key in Settings to enable AI scanning</div>}
    </div>); })()}

    {/* HIPAA Warning Modal */}
    {showHipaaWarn && <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.75)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ maxWidth: 400, width: "100%", background: "#1e293b", borderRadius: 16, border: "1px solid #f59e0b", overflow: "hidden" }}>
        ...
        <div style={{ padding: "16px 20px" }}>
          ...
          <div style={{ fontSize: 13, color: "#e2e8f0", lineHeight: 1.6, marginBottom: 12 }}>
            <span style={{ fontWeight: 600, color: "#f8fafc" }}>Safeguards in place:</span>
          </div>
          <div style={{ fontSize: 12, color: "#94a3b8", lineHeight: 1.7, marginBottom: 12, paddingLeft: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={function() { setShowHipaaWarn(false); pendingScanType.current = null; }} style={{ flex: 1, padding: "12px 16px", borderRadius: 10, border: "1px solid #334155", background: "#0f172a", color: "#94a3b8", fontSize: 14, cursor: "pointer" }}>Cancel</button>
            <button onClick={function() { setHipaaAck(true); setShowHipaaWarn(false); setTimeout(function() { if (pendingScanType.current === "opnote") { imgRef.current && imgRef.current.click(); } else if (pendingScanType.current === "clinic") { clinicRef.current && clinicRef.current.click(); } pendingScanType.current = null; }, 100); }} style={{ flex: 1, padding: "12px 16px", borderRadius: 10, border: "none", background: "#f59e0b", color: "#0f172a", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>I Understand</button>
          </div>
        </div>
      </div>
    </div>}
    ...
    {/* Clinic Schedule Results Panel */}
    {clinicResults && <div style={{ marginBottom: 12, borderRadius: 12, border: "1px solid #10b981", background: "rgba(16,185,129,0.03)", overflow: "hidden" }}>
      <div style={{ padding: "10px 14px", background: "rgba(16,185,129,0.08)", borderBottom: "1px solid rgba(16,185,129,0.2)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ fontSize: 14, fontWeight: 700, color: "#10b981" }}>Clinic Encounters</div>
          <div style={{ fontSize: 11, color: "#64748b" }}>{clinicResults.filter(function(e) { return e.selected; }).length} of {clinicResults.length} selected</div>
        </div>
        <div style={{ display: "flex", gap: 6 }}>
          <button onClick={function() { setClinicResults(null); }} style={{ padding: "6px 12px", borderRadius: 8, border: "1px solid #334155", background: "#1e293b", color: "#94a3b8", fontSize: 12, cursor: "pointer" }}>Cancel</button>
          <button onClick={saveClinicBatch} style={{ padding: "6px 12px", borderRadius: 8, border: "none", background: "#10b981", color: "#fff", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Log All</button>
        </div>
      </div>
      <div style={{ maxHeight: 350, overflowY: "auto" }}>
        {clinicResults.map(function(enc, idx) {
          var encRVU = enc.codes.reduce(function(s, c) { return s + (c.valid ? c.wRVU : 0); }, 0);
          return (<div key={enc.id} style={{ padding: "10px 14px", borderBottom: "1px solid rgba(51,65,85,0.3)", opacity: enc.selected ? 1 : 0.4 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span onClick={function() { toggleClinicEnc(idx); }} style={{ width: 20, height: 20, borderRadius: 4, border: enc.selected ? "2px solid #10b981" : "2px solid #334155", background: enc.selected ? "rgba(16,185,129,0.2)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 12, color: "#10b981" }}>{enc.selected ? "\u2713" : ""}</span>
                <span style={{ fontSize: 13, fontWeight: 600, color: "#f8fafc" }}>{enc.patient}</span>
                <span style={{ fontSize: 11, color: "#64748b" }}>{enc.visitType}</span>
              </div>
              <span style={{ fontSize: 13, fontFamily: "JetBrains Mono", color: "#10b981", fontWeight: 600 }}>{encRVU.toFixed(2)}</span>
            </div>
            {enc.codes.map(function(c, ci) {
              return (<div key={ci} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "3px 0 3px 28px" }}>
                <div>
                  <span style={{ fontSize: 12, fontFamily: "JetBrains Mono", color: c.valid ? "#0ea5e9" : "#ef4444", fontWeight: 600 }}>{c.code}</span>
                  <span style={{ fontSize: 11, color: "#94a3b8", marginLeft: 6 }}>{c.description.length > 35 ? c.description.slice(0, 35) + "..." : c.description}</span>
                  {!c.valid && <span style={{ fontSize: 10, color: "#ef4444", marginLeft: 4 }}>(not found)</span>}
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 9, padding: "1px 5px", borderRadius: 4, background: c.confidence === "high" ? "rgba(16,185,129,0.15)" : c.confidence === "medium" ? "rgba(245,158,11,0.15)" : "rgba(239,68,68,0.15)", color: c.confidence === "high" ? "#10b981" : c.confidence === "medium" ? "#f59e0b" : "#ef4444" }}>{c.confidence}</span>
                  <span style={{ fontSize: 11, fontFamily: "JetBrains Mono", color: "#e2e8f0" }}>{c.wRVU.toFixed(2)}</span>
                </div>
              </div>);
            })}
          </div>);
      ...
      <div style={{ padding: "10px 14px", background: "rgba(16,185,129,0.05)", borderTop: "1px solid rgba(16,185,129,0.2)", display: "flex", justifyContent: "space-between" }}>
        <span style={{ fontSize: 12, color: "#94a3b8" }}>Total selected</span>
        <span style={{ fontSize: 14, fontFamily: "JetBrains Mono", color: "#10b981", fontWeight: 700 }}>{clinicResults.filter(function(e) { return e.selected; }).reduce(function(s, e) { return s + e.codes.reduce(function(cs, c) { return cs + (c.valid ? c.wRVU : 0); }, 0); }, 0).toFixed(2)} wRVUs</span>
      </div>
    </div>}
    ...
    <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
      <button onClick={function() { setIsCall(false); }} style={{ flex: 1, padding: "10px 12px", borderRadius: 10, border: !isCall ? "2px solid #0ea5e9" : "1px solid #334155", background: !isCall ? "rgba(14,165,233,0.1)" : "#1e293b", color: !isCall ? "#0ea5e9" : "#64748b", fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
      ...
      <button onClick={function() { setIsCall(true); }} style={{ flex: 1, padding: "10px 12px", borderRadius: 10, border: isCall ? "2px solid #a78bfa" : "1px solid #334155", background: isCall ? "rgba(139,92,246,0.1)" : "#1e293b", color: isCall ? "#a78bfa" : "#64748b", fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
    ...
    <div style={{ display: "flex", gap: 4, marginBottom: 8, flexWrap: "wrap" }}>
      {procs.map(function(p, i) {
        ...
        return (<button key={p.id} onClick={function() { setActiveProc(i); }} style={{
          padding: "6px 12px", borderRadius: 8, border: isActive ? "2px solid #0ea5e9" : "1px solid #334155",
          background: isActive ? "rgba(14,165,233,0.1)" : "#1e293b", color: isActive ? "#0ea5e9" : "#94a3b8",
          fontSize: 12, fontFamily: "JetBrains Mono", cursor: "pointer", fontWeight: isActive ? 700 : 400
        }}>
          {info ? info.code : "Proc " + (i + 1)}
          {procs.length > 1 && <span onClick={function(ev) { ev.stopPropagation(); removeProc(i); }} style={{ marginLeft: 6, color: "#64748b", fontSize: 10 }}>x</span>}
        </button>);
      })}
      <button onClick={addProc} style={{ padding: "6px 12px", borderRadius: 8, border: "1px dashed #334155", background: "transparent", color: "#64748b", fontSize: 12, cursor: "pointer" }}>+ Add</button>
    </div>
    ...
    {!curSel && showFavs && !curSearch.trim() && <div style={S.resultsList}>
      {favCodes.length === 0 && mostUsed.length === 0 && <div style={{ padding: 20, textAlign: "center", color: "#64748b" }}>
        <div style={{ fontSize: 28, marginBottom: 8 }}>{"\u2606"}</div>
        <div style={{ fontSize: 14, fontWeight: 600, color: "#94a3b8", marginBottom: 4 }}>No favorites yet</div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 4 }}>
              <span style={{ fontSize: 11, color: "#94a3b8" }}>{cpt.category}</span>
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4, flexShrink: 0, marginLeft: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <div style={{ textAlign: "right" }}>
                ...
                <span style={{ fontSize: 10, color: "#64748b" }}> wRVU</span>
              </div>
            ...
            <div style={{ display: "flex", gap: 2 }}>
              {idx > 0 && <span onClick={function(ev) { ev.stopPropagation(); moveFav(cpt.code, -1); }} style={{ fontSize: 14, cursor: "pointer", color: "#475569", padding: "0 4px", lineHeight: 1 }}>{"\u25B2"}</span>}
              {idx < favCodes.length - 1 && <span onClick={function(ev) { ev.stopPropagation(); moveFav(cpt.code, 1); }} style={{ fontSize: 14, cursor: "pointer", color: "#475569", padding: "0 4px", lineHeight: 1 }}>{"\u25BC"}</span>}
            </div>
          </div>
        </div>
      </button>); })}
      {mostUsed.length > 0 && <>
        <div style={{ padding: "12px 0 6px", display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: favCodes.length > 0 ? "1px solid #334155" : "none", marginTop: favCodes.length > 0 ? 8 : 0 }}>
          <span style={{ fontSize: 11, fontWeight: 600, color: "#0ea5e9", textTransform: "uppercase", letterSpacing: 1 }}>Most Used</span>
          <span style={{ fontSize: 10, color: "#475569" }}>Based on your history</span>
        </div>
          ...
          return (<button key={cpt.code} onClick={function() { selectCode(cpt); setShowFavs(false); }} style={{ ...S.resultItem, borderLeft: "3px solid rgba(14,165,233,0.3)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ ...S.resultDesc, marginTop: 3 }}>{cpt.desc}</div>
                <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 3 }}>{cpt.category}</div>
              </div>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4, flexShrink: 0, marginLeft: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <div style={{ textAlign: "right" }}>
                  ...
                  <span onClick={function(ev) { ev.stopPropagation(); toggleFav(cpt.code); }} style={{ fontSize: 18, cursor: "pointer", color: "#334155", lineHeight: 1 }}>{"\u2606"}</span>
                </div>
                <span style={{ fontSize: 10, fontFamily: "JetBrains Mono", color: "#10b981" }}>{"$"}{comp.toFixed(0)}</span>
              </div>
            </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 2, flexShrink: 0, marginLeft: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <span onClick={function(ev) { ev.stopPropagation(); toggleFav(cpt.code); }} style={{ fontSize: 18, cursor: "pointer", color: isFav(cpt.code) ? "#fbbf24" : "#334155", lineHeight: 1 }}>{isFav(cpt.code) ? "\u2605" : "\u2606"}</span>
          </div>
          <span style={{ fontSize: 10, fontFamily: "JetBrains Mono", color: "#10b981" }}>{"$"}{comp.toFixed(0)}</span>
        </div>
      </div>
    </button>); })}{filtered.length === 0 && curSearch && <div style={{ padding: 20, textAlign: "center", color: "#64748b" }}>No matching CPT codes found</div>}</div>}

    {curSel && <><div style={S.selectedCard}><div style={{ display: "flex", justifyContent: "space-between" }}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={S.selectedCode}>{curSel.code}</span><span onClick={function() { toggleFav(curSel.code); }} style={{ fontSize: 22, cursor: "pointer", color: isFav(curSel.code) ? "#fbbf24" : "#475569", lineHeight: 1 }}>{isFav(curSel.code) ? "\u2605" : "\u2606"}</span>{usageCounts[curSel.code] > 0 && <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 10, background: "rgba(14,165,233,0.12)", color: "#38bdf8", fontFamily: "JetBrains Mono", fontWeight: 600 }}>{"x"}{usageCounts[curSel.code]}</span>}</div><button onClick={clearCode} style={S.clearBtn}>x</button></div><div style={S.selectedDesc}>{curSel.desc}</div><div style={{ display: "flex", gap: 16, marginTop: 8 }}><div><div style={{ fontSize: 11, color: "#64748b" }}>Base wRVU</div><div style={{ fontSize: 18, fontFamily: "JetBrains Mono", color: "#e2e8f0", fontWeight: 600 }}>{curSel.wRVU}</div></div>{curMods.length > 0 && <div><div style={{ fontSize: 11, color: "#64748b" }}>Adjusted</div><div style={{ fontSize: 18, fontFamily: "JetBrains Mono", color: "#0ea5e9", fontWeight: 600 }}>{calcProcRVU(cur).toFixed(2)}</div></div>}<div><div style={{ fontSize: 11, color: "#64748b" }}>Compensation</div><div style={{ fontSize: 18, fontFamily: "JetBrains Mono", color: "#10b981", fontWeight: 600 }}>${(calcProcRVU(cur) * data.settings.ratePerRVU).toFixed(0)}</div></div></div></div>
    ...
    {/* Encounter summary */}
    {canSave && <div style={{ ...S.card, background: "linear-gradient(135deg, #1e293b, #0f172a)", border: isCall ? "1px solid rgba(139,92,246,0.3)" : "1px solid #334155", marginTop: 8 }}>
      ...
      <div style={{ marginTop: 8 }}>{procs.filter(function(p){return p.code;}).map(function(p, i) {
        ...
        return (<div key={p.id} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: i < procs.filter(function(x){return x.code;}).length - 1 ? "1px solid rgba(51,65,85,0.5)" : "none" }}>
          <div><span style={{ fontSize: 12, fontFamily: "JetBrains Mono", color: "#0ea5e9", fontWeight: 600 }}>{p.code}</span>{p.mods.map(function(m) { return <span key={m} style={{ fontSize: 10, color: "#fbbf24", marginLeft: 4 }}>{m}</span>; })}<div style={{ fontSize: 11, color: "#94a3b8" }}>{info ? info.desc : ""}</div></div>
          <div style={{ fontSize: 13, fontFamily: "JetBrains Mono", color: "#e2e8f0", fontWeight: 600 }}>{adj.toFixed(2)}</div>
        </div>);
      })}</div>
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8, paddingTop: 8, borderTop: "1px solid #334155" }}>
        ...
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 18, fontFamily: "JetBrains Mono", color: "#f8fafc", fontWeight: 700 }}>{totalRVU.toFixed(2)} wRVU</div>
