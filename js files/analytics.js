...
function Analytics({ data, db, setView }) {
  ...
  return (<div style={S.page}>
    ...
    {/* Goal Pace Card */}
    {goal > 0 && <div style={{ ...S.card, background: "linear-gradient(135deg, #1e293b, #0f172a)", border: "1px solid #334155" }}>
      ...
      <div style={{ marginTop: 8 }}>{topProcs.map(function(p, i) {
        return (<div key={p.code} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: i < topProcs.length - 1 ? "1px solid rgba(51,65,85,0.3)" : "none" }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div><span style={{ fontSize: 12, fontFamily: "JetBrains Mono", color: "#0ea5e9", fontWeight: 600 }}>{p.code}</span><span style={{ fontSize: 11, color: "#64748b", marginLeft: 6 }}>x{p.count}</span></div>
            <div style={{ fontSize: 11, color: "#94a3b8", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.desc}</div>
          </div>
          <div style={{ fontSize: 13, fontFamily: "JetBrains Mono", color: "#e2e8f0", fontWeight: 600, marginLeft: 8 }}>{p.totalRVU.toFixed(1)}</div>
        </div>);
      })}</div>
    </div>}

    {/* Category Breakdown - Enhanced */}
    {topCats.length > 0 && <div style={{ ...S.card, background: "linear-gradient(135deg, #1e293b, #0f172a)", border: "1px solid #334155", padding: 16 }}>
      <div style={{ fontSize: 14, fontWeight: 700, color: "#f8fafc", marginBottom: 4 }}>wRVU by Category</div>
      <div style={{ fontSize: 11, color: "#64748b", marginBottom: 14 }}>Total RVUs and top case per category</div>
        ...
        return (<div key={c.name} style={{ padding: "10px 0", borderBottom: i < topCats.length - 1 ? "1px solid rgba(51,65,85,0.3)" : "none" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 6 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ fontSize: 13, color: "#f8fafc", fontWeight: 600 }}>{c.name}</span>
            </div>
            <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
              <span style={{ fontSize: 11, color: "#64748b" }}>{c.count} case{c.count !== 1 ? "s" : ""}</span>
          ...
          {topCase && <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "4px 8px", background: "rgba(15,23,42,0.6)", borderRadius: 6, marginLeft: 18 }}>
            ...
            <span style={{ fontSize: 12, fontFamily: "JetBrains Mono", color: "#e2e8f0", fontWeight: 600 }}>{topCaseRVU.toFixed(2)}</span>
          </div>}
        </div>);
      })}</div>
      <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid #334155", display: "flex", justifyContent: "space-between" }}>
        ...
        <span style={{ fontSize: 16, fontFamily: "JetBrains Mono", color: "#f8fafc", fontWeight: 700 }}>{topCats.reduce(function(s, c) { return s + c.rvu; }, 0).toFixed(1)} wRVUs</span>
      </div>
    </div>}
    ...
    {showSummary && <div style={{ ...S.card, border: "1px solid rgba(139,92,246,0.25)" }}>
      ...
      {summaryData.privateCases + summaryData.callCases > 0 && <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
        <div style={{ flex: 1, padding: "6px 10px", borderRadius: 6, background: "rgba(14,165,233,0.06)", border: "1px solid rgba(14,165,233,0.15)" }}><div style={{ fontSize: 10, color: "#0ea5e9", fontWeight: 600 }}>Private</div><div style={{ fontSize: 13, fontFamily: "JetBrains Mono", color: "#e2e8f0" }}>{summaryData.privateRVU.toFixed(1)} wRVU | {summaryData.privateCases} cases</div></div>
      ...
      {summaryData.topProcs.length > 0 && <div style={{ marginBottom: 12 }}>
        <div style={{ fontSize: 11, fontWeight: 600, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 6 }}>Top Procedures</div>
        {summaryData.topProcs.map(function(p) { return (<div key={p.code} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "4px 0", borderBottom: "1px solid rgba(51,65,85,0.3)" }}><div><span style={{ fontSize: 12, fontFamily: "JetBrains Mono", color: "#0ea5e9", fontWeight: 600 }}>{p.code}</span><span style={{ fontSize: 10, color: "#64748b", marginLeft: 4 }}>x{p.count}</span><div style={{ fontSize: 11, color: "#94a3b8" }}>{p.desc.length > 35 ? p.desc.slice(0, 35) + "..." : p.desc}</div></div><span style={{ fontSize: 12, fontFamily: "JetBrains Mono", color: "#e2e8f0", fontWeight: 600, flexShrink: 0, marginLeft: 8 }}>{p.rvu.toFixed(1)}</span></div>); })}
      </div>}
