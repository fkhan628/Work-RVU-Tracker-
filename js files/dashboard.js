...
function Dashboard({ data, db, setView }) {
  ...
  const ye = entries.filter(function(e) { var d = new Date(e.date); return d >= ys && d < yearBounds.end; });
  ...
  for (var i = chartMonths - 1; i >= 0; i--) {
    ...
    var trackedMonth = entries.filter(function(e) { var ed = new Date(e.date); return ed.getMonth() === md.getMonth() && ed.getFullYear() === md.getFullYear(); });
  ...
  const wRVU = isCurrent ? entries.filter(e => new Date(e.date) >= ws).reduce((s, e) => s + e.adjustedRVU, 0) : 0;
  ...
  var prevYearRVU = useMemo(function() {
    ...
    var prevEntries = entries.filter(function(e) { var d = new Date(e.date); return d >= prevStart && d < prevEnd; });
