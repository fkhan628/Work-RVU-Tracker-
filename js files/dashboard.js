...
function Dashboard({ data, db, setView }) {
  ...
  var prevYearRVU = useMemo(function() {
    ...
    if (prevInstTotal > 0 || prevTracked > 0) {
      return { total: hasInst ? prevInstTotal : prevTracked, hasData: true };
    }
  ...
  return (<div style={S.page}>
    ...
    <div style={S.cardMain}><div style={{ display: "flex", alignItems: "center", gap: 24 }}>
      <svg width="100" height="100" viewBox="0 0 100 100"><circle cx="50" cy="50" r="42" fill="none" stroke="var(--bg-card)" strokeWidth="8" /><circle cx="50" cy="50" r="42" fill="none" stroke={pace === "ahead" ? "#10b981" : "#f59e0b"} strokeWidth="8" strokeLinecap="round" strokeDasharray={gPct * 2.639 + " 263.9"} transform="rotate(-90 50 50)" style={{ transition: "stroke-dasharray 0.8s ease" }} /><text x="50" y="46" textAnchor="middle" fill="var(--text-bright)" fontSize="18" fontFamily="JetBrains Mono" fontWeight="600">{gPct.toFixed(0)}%</text><text x="50" y="62" textAnchor="middle" fill="var(--text-muted)" fontSize="9" fontFamily="DM Sans">of goal</text></svg>
      <div><div style={S.metricBig}>{tRVU.toFixed(1)}</div><div style={S.metricLabel}>wRVUs {isCurrent ? "YTD" : yearBounds.label}</div>
        {hasInst && <div style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 1 }}>W: {instWork.toFixed(0)} | C: {instSplit.toFixed(0)}</div>}
    ...
    {hasInst && <div style={{ marginTop: 10, padding: "6px 10px", borderRadius: 8, background: "rgba(139,92,246,0.08)", border: "1px solid rgba(139,92,246,0.15)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      ...
      <span style={{ fontSize: 10, color: "var(--text-dim)" }}>Projected: {projected.toFixed(0)} wRVUs/yr</span>
    </div>}
    </div>
    ...
    <div style={S.statsRow}>
      {isCurrent ? <>
        ...
        <div style={S.statCard}><div style={S.statValue}>{ye.length}</div><div style={S.statLabel}>Cases</div></div>
        <div style={S.statCard}><div style={S.statValue}>{ye.length > 0 ? (tRVU / ye.length).toFixed(1) : "0"}</div><div style={S.statLabel}>Avg/Case</div></div>
      </>}
    ...
    {prevYearRVU.hasData && <div style={{ ...S.card, border: "1px solid rgba(139,92,246,0.2)", background: "rgba(139,92,246,0.02)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: "var(--text-dim)", marginTop: 2 }}>Same period last year: {prevYearRVU.total.toFixed(0)} wRVUs</div>
        </div>
        <div style={{ textAlign: "right" }}>
          {(function() {
            ...
            return <>
              ...
              <div style={{ fontSize: 10, fontFamily: "JetBrains Mono", color: "var(--text-dim)" }}>{yoyDiff >= 0 ? "+" : ""}{yoyPct.toFixed(1)}%</div>
            </>;
          })()}
        </div>
      </div>
    ...
    <div style={{ display: "flex", alignItems: "flex-end", gap: 8, height: 110, marginTop: 12 }}>{months.map(function(m, i) {
      ...
      return (<div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
        <span style={{ fontSize: 10, fontFamily: "JetBrains Mono", color: "var(--text-secondary)", fontWeight: 500 }}>{m.primary > 0 ? m.primary.toFixed(0) : ""}</span>
        <div style={{ width: "100%", position: "relative" }}>
          {hasInst && <div style={{ width: "100%", height: Math.max(instH > 0 ? instH : 0, instH > 0 ? 4 : 0), background: isLast ? "linear-gradient(to top, #8b5cf6, #a78bfa)" : "linear-gradient(to top, var(--border-default), var(--text-faint))", borderRadius: 4, transition: "height 0.3s" }} />}
          ...
          {!hasInst && <div style={{ width: "100%", height: Math.max(4, trackH > 0 ? trackH : 0), background: isLast ? "linear-gradient(to top, #0ea5e9, #38bdf8)" : "linear-gradient(to top, var(--border-default), var(--text-faint))", borderRadius: 4, transition: "height 0.3s" }} />}
        </div>
    ...
    {!hasInst && <div style={{ ...S.card, border: "1px solid rgba(139,92,246,0.2)", background: "rgba(139,92,246,0.03)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: "var(--text-dim)" }}>Go to Compare tab to import your institution's RVU data for goal tracking</div>
        </div>
      </div>
    </div>}

    <div style={{ ...S.card, background: "var(--bg-inset)", border: "1px solid var(--border-subtle)", padding: "10px 16px" }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><span style={{ fontSize: 11, color: "var(--text-faint)" }}>Data: {hasInst ? "Institution" : "Self-tracked"} | CMS {DATA_YEAR}</span><span style={{ fontSize: 10, color: "var(--text-ghost)", fontFamily: "JetBrains Mono" }}>{DATA_VERSION}</span></div></div>
