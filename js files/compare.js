...
function Compare({ data, upd, setView }) {
  ...
  return (<div style={S.page}>
    ...
    {instData.length > 0 && <>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
        <div style={{ ...S.card, border: "1px solid rgba(139,92,246,0.3)" }}>
          ...
          <div style={{ fontSize: 10, color: "#64748b", marginTop: 2 }}>Work: {totals.instWork.toFixed(1)} | Call: {totals.instSplit.toFixed(1)}</div>
        </div>
        <div style={{ ...S.card, border: "1px solid rgba(14,165,233,0.3)" }}>
          ...
          <div style={{ fontSize: 10, color: "#64748b", marginTop: 2 }}>{totals.measCases} cases logged</div>
        </div>
      </div>
      <div style={{ ...S.card, border: diff >= 0 ? "1px solid rgba(16,185,129,0.3)" : "1px solid rgba(245,158,11,0.3)", marginBottom: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 2 }}>{diff >= 0 ? "You tracked more than institution reported" : "Institution reported more than you tracked"}</div>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 22, fontFamily: "JetBrains Mono", color: diff >= 0 ? "#10b981" : "#f59e0b", fontWeight: 700 }}>{diff >= 0 ? "+" : ""}{diff.toFixed(1)}</div>
            <div style={{ fontSize: 11, fontFamily: "JetBrains Mono", color: "#64748b" }}>{diff >= 0 ? "+" : ""}{diffPct.toFixed(1)}%</div>
          </div>
        </div>
      </div>
    </>}
        ...
        <div style={{ overflowX: "auto", marginTop: 8 }}>
          <svg viewBox={"0 0 " + W + " " + H} style={{ width: "100%", maxWidth: W, height: "auto", display: "block" }}>
            ...
            {gridLines.map(function(g, i) {
              return (<g key={i}>
                <line x1={padL} y1={g.y} x2={W - padR} y2={g.y} stroke="#1e293b" strokeWidth="1" />
                <text x={padL - 4} y={g.y + 3} textAnchor="end" fill="#475569" fontSize="9" fontFamily="JetBrains Mono">{g.label}</text>
              </g>);
            })}
            ...
            {pts.map(function(p, i) {
              return <circle key={"i" + i} cx={xOf(i)} cy={yOf(p.inst)} r={hoverIdx === i ? 5 : 3} fill="#a78bfa" stroke="#0f172a" strokeWidth="1.5" />;
            })}

            {/* Data points - Tracked */}
            {pts.map(function(p, i) {
              if (p.meas === 0) return null;
              return <circle key={"m" + i} cx={xOf(i)} cy={yOf(p.meas)} r={hoverIdx === i ? 5 : 3} fill="#0ea5e9" stroke="#0f172a" strokeWidth="1.5" />;
            })}
            ...
            {hoverIdx >= 0 && hoverIdx < pts.length && (function() {
              ...
              return (<g>
                <line x1={tx} y1={padT} x2={tx} y2={padT + cH} stroke="#334155" strokeWidth="1" strokeDasharray="3,3" />
                <rect x={flipLeft ? tx - 108 : tx + 8} y={padT} width={100} height={52} rx="4" fill="#0f172a" stroke="#334155" strokeWidth="1" opacity="0.95" />
                <text x={flipLeft ? tx - 103 : tx + 13} y={padT + 12} fill="#94a3b8" fontSize="9" fontFamily="DM Sans">{monthLabel(hp.month)}</text>
            ...
            {pts.map(function(p, i) {
              ...
              return <text key={"xl" + i} x={xOf(i)} y={H - 5} textAnchor="middle" fill="#475569" fontSize={n > 18 ? "7" : "8"} fontFamily="JetBrains Mono">{n > 18 ? monthLabelShort(p.month) : monthLabel(p.month)}</text>;
            })}
          </svg>
        </div>
        <div style={{ display: "flex", gap: 16, justifyContent: "center", marginTop: 6 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 4 }}><div style={{ width: 16, height: 3, borderRadius: 2, background: "#a78bfa" }}></div><span style={{ fontSize: 10, color: "#94a3b8" }}>Institution</span></div>
          <div style={{ display: "flex", alignItems: "center", gap: 4 }}><div style={{ width: 16, height: 3, borderRadius: 2, background: "#0ea5e9" }}></div><span style={{ fontSize: 10, color: "#94a3b8" }}>Tracked</span></div>
        </div>
      </div>);
    })()}
    ...
    {xlPreview && <div style={{ ...S.card, border: "1px solid rgba(16,185,129,0.3)", marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: "#64748b" }}>{xlPreview.length} months found</div>
        </div>
        <button onClick={function() { setXlPreview(null); setXlRawWb(null); setXlSheets([]); setXlError(""); }} style={{ padding: "4px 10px", borderRadius: 6, border: "1px solid #334155", background: "transparent", color: "#94a3b8", fontSize: 11, cursor: "pointer" }}>Cancel</button>
      </div>

      {/* Sheet selector */}
      {xlSheets.length > 1 && <div style={{ marginTop: 8 }}>
        <div style={{ fontSize: 10, color: "#64748b", marginBottom: 4 }}>Sheet:</div>
        <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
          {xlSheets.map(function(name, idx) {
            return <button key={idx} onClick={function() { switchXlSheet(idx); }} style={{ padding: "4px 10px", borderRadius: 6, border: idx === xlSheet ? "1px solid #10b981" : "1px solid #334155", background: idx === xlSheet ? "rgba(16,185,129,0.1)" : "transparent", color: idx === xlSheet ? "#10b981" : "#94a3b8", fontSize: 11, cursor: "pointer" }}>{name}</button>;
          })}
        </div>
      </div>}
      ...
      <div style={{ maxHeight: 280, overflowY: "auto", marginTop: 10, scrollbarWidth: "thin" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr", gap: 0, fontSize: 10, color: "#64748b", padding: "4px 0", borderBottom: "1px solid #334155", fontWeight: 600 }}>
        ...
        {xlPreview.map(function(d) {
          ...
          return (<div key={d.month} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr", gap: 0, fontSize: 12, padding: "6px 0", borderBottom: "1px solid rgba(51,65,85,0.3)", alignItems: "center" }}>
            <span style={{ color: "#f8fafc", fontFamily: "JetBrains Mono", fontSize: 11 }}>{d.month}{existing && <span style={{ fontSize: 8, color: "#f59e0b", marginLeft: 3 }} title="Will update existing">{"\u25CF"}</span>}</span>
            <span style={{ textAlign: "right", color: "#a78bfa", fontFamily: "JetBrains Mono" }}>{d.workRVU.toFixed(1)}</span>
            <span style={{ textAlign: "right", color: "#94a3b8", fontFamily: "JetBrains Mono" }}>{d.splitRVU.toFixed(1)}</span>
            <span style={{ textAlign: "right", color: "#e2e8f0", fontFamily: "JetBrains Mono", fontWeight: 600 }}>{total.toFixed(1)}</span>
            <span style={{ textAlign: "right", color: "#64748b", fontFamily: "JetBrains Mono" }}>{d.cases || "-"}</span>
          </div>);
        })}
      </div>

      {/* Import totals and buttons */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr", gap: 0, fontSize: 11, padding: "8px 0 4px", borderTop: "1px solid #334155", marginTop: 4, fontWeight: 600 }}>
        ...
        <span style={{ textAlign: "right", color: "#94a3b8", fontFamily: "JetBrains Mono" }}>{xlPreview.reduce(function(s, d) { return s + d.splitRVU; }, 0).toFixed(0)}</span>
        <span style={{ textAlign: "right", color: "#e2e8f0", fontFamily: "JetBrains Mono" }}>{xlPreview.reduce(function(s, d) { return s + d.workRVU + d.splitRVU; }, 0).toFixed(0)}</span>
        <span style={{ textAlign: "right", color: "#64748b", fontFamily: "JetBrains Mono" }}>{xlPreview.reduce(function(s, d) { return s + (d.cases || 0); }, 0)}</span>
      </div>
    ...
    {showAdd && <div style={{ ...S.card, border: "1px solid rgba(139,92,246,0.3)", marginBottom: 12 }}>
      ...
      <div style={{ fontSize: 12, color: "#64748b", marginBottom: 8 }}>Total: <span style={{ fontFamily: "JetBrains Mono", color: "#a78bfa", fontWeight: 600 }}>{((parseFloat(editWork) || 0) + (parseFloat(editSplit) || 0)).toFixed(1)} wRVUs</span></div>
        ...
        {displayData.map(function(d, dispIdx) {
          ...
          return (<div key={d.month} style={{ padding: "10px 0", borderBottom: dispIdx < displayData.length - 1 ? "1px solid rgba(51,65,85,0.5)" : "none" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: "#f8fafc" }}>{monthLabel(d.month)}</div>
                <div style={{ display: "flex", gap: 12, marginTop: 4 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 9, color: "#64748b" }}>W:{(d.workRVU || 0).toFixed(0)} C:{(d.splitRVU || 0).toFixed(0)}{d.cases ? " | " + d.cases + " cases" : ""}</div>
                  </div>
                  <div>
                    ...
                    <div style={{ fontSize: 9, color: "#64748b" }}>{m ? m.cases + " cases" : "No data"}</div>
                  </div>
                  <div>
              ...
              <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                <button onClick={function() { startEdit(realIdx); }} style={{ ...S.deleteBtn, color: "#94a3b8" }}>{"\u270E"}</button>
    ...
    {instData.length > 0 && settings.ratePerRVU > 0 && <div style={{ ...S.card, border: "1px solid rgba(16,185,129,0.3)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 8 }}>
        <div>
          <div style={{ fontSize: 10, color: "#64748b" }}>Institution Based</div>
        ...
        <div>
          <div style={{ fontSize: 10, color: "#64748b" }}>Your Tracked</div>
      ...
      <div style={{ fontSize: 10, color: "#475569", marginTop: 6 }}>Based on {"$"}{settings.ratePerRVU}/wRVU rate</div>
    </div>}

    {instData.length === 0 && !showAdd && !xlPreview && <div style={{ ...S.card, textAlign: "center", padding: 30 }}>
      <div style={{ fontSize: 28, marginBottom: 8 }}>{"\u2194"}</div>
      <div style={{ fontSize: 14, fontWeight: 600, color: "#94a3b8", marginBottom: 4 }}>No Institution Data Yet</div>
      <div style={{ fontSize: 12, color: "#64748b", lineHeight: 1.5 }}>Upload your Excel spreadsheet or add months manually to compare institution RVUs against what you're tracking.</div>
    </div>}
  </div>);
}
