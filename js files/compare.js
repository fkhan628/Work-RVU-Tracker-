...
// =======================================
function Compare({ data, upd, setView, showComp }) {
  ...
  var [viewYear, setViewYear] = useState(function() { return String(new Date().getFullYear()); });
  var [showMonthlyDetail, setShowMonthlyDetail] = useState(false);
  ...
  return (<div style={S.page}>
    ...
    {/* Year filter: current year, -1, -2 pinned; older years in dropdown; All */}
    {years.length > 0 && (function() {
      var nowY = new Date().getFullYear();
      var pinned = [String(nowY), String(nowY - 1), String(nowY - 2)];
      var older = years.filter(function(y) { return pinned.indexOf(y) === -1; });
      var olderSelected = older.indexOf(viewYear) !== -1;
      return (<div style={S.catRow}>
        {pinned.map(function(y) { return (
          <button key={y} onClick={function() { setViewYear(y); }} style={viewYear === y ? S.catBtnActive : S.catBtn}>{y}</button>
        ); })}
        {older.length > 0 && <select
          value={olderSelected ? viewYear : ""}
          onChange={function(e) { if (e.target.value) setViewYear(e.target.value); }}
          style={{
        ...
        >
          <option value="">{olderSelected ? viewYear : "Older"}</option>
    ...
    {/* Monthly detail list (collapsible) */}
    {displayData.length > 0 && <div style={S.card}>
      <button
        onClick={function() { setShowMonthlyDetail(function(v) { return !v; }); }}
        style={{
      ...
      >
        <span style={S.cardLabel}>Monthly Detail <span style={{ color: "var(--text-dim)", fontWeight: 400, textTransform: "none", letterSpacing: 0 }}>({displayData.length})</span></span>
        <span style={{ color: "var(--text-muted)", fontSize: 14, transition: "transform 0.15s", transform: showMonthlyDetail ? "rotate(90deg)" : "rotate(0deg)", display: "inline-block" }}>{"\u203A"}</span>
      </button>
      {showMonthlyDetail && <div style={{ marginTop: 8 }}>
    ...
    {instData.length > 0 && settings.ratePerRVU > 0 && <div style={{ ...S.card, border: "1px solid rgba(16,185,129,0.3)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 8 }}>
        <div>
          ...
          <div style={{ fontSize: 18, fontFamily: "JetBrains Mono", color: "#a78bfa", fontWeight: 600 }}>{fmtDollar(totals.instTotal * settings.ratePerRVU, showComp)}</div>
        </div>
        <div>
          <div style={{ fontSize: 10, color: "var(--text-dim)" }}>Your Tracked</div>
          <div style={{ fontSize: 18, fontFamily: "JetBrains Mono", color: "#0ea5e9", fontWeight: 600 }}>{fmtDollar(totals.measRVU * settings.ratePerRVU, showComp)}</div>
        </div>
      </div>
      <div style={{ fontSize: 10, color: "var(--text-faint)", marginTop: 6 }}>{showComp ? "Based on $" + settings.ratePerRVU + "/wRVU rate" : ""}</div>
    </div>}
