...
function Settings({ data, db, cptMap, categories, upd, setView, theme, toggleTheme }) {
  ...
  const [bioEnabled, setBioEnabled] = useState(function() { return window.RVU_CRYPTO && window.RVU_CRYPTO.hasBiometrics(); });
  const [bioStatus, setBioStatus] = useState("");

  useEffect(function() {
    if (window.RVU_CRYPTO && window.RVU_CRYPTO.biometricsAvailable) {
      window.RVU_CRYPTO.biometricsAvailable().then(function(avail) { setBioAvailable(avail); });
    }
  }, []);
  ...
  return (<div style={S.page}>
    ...
    <div style={{ ...S.card, border: "1px solid rgba(16,185,129,0.2)" }}>
      ...
      {bioAvailable && <div style={{ marginTop: 10, padding: "10px 12px", background: "rgba(15,23,42,0.5)", borderRadius: 8, border: "1px solid #1e293b" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ fontSize: 12, color: "#94a3b8", fontWeight: 600 }}>{"\uD83E\uDDEC"} Biometric Unlock</div>
            <div style={{ fontSize: 11, color: "#475569", marginTop: 2 }}>Use Face ID or fingerprint to unlock</div>
          </div>
          ...
          }} style={{ padding: "8px 16px", borderRadius: 10, border: bioEnabled ? "1px solid rgba(16,185,129,0.3)" : "1px solid #334155", background: bioEnabled ? "rgba(16,185,129,0.08)" : "#1e293b", color: bioEnabled ? "#10b981" : "#94a3b8", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
