...
(function() {
  ...
  // --- Uint8Array to base64 (chunked to avoid call stack overflow) ---
  function u8toB64(arr) {
    var chunks = [], sz = 8192;
    for (var i = 0; i < arr.length; i += sz) {
      chunks.push(String.fromCharCode.apply(null, arr.subarray(i, i + sz)));
    }
  ...
  // =======================================
  var BIO_ENABLED_KEY = "rvu-bio-enabled";
  var BIO_CRED_KEY = "rvu-bio-cred";
  var BIO_PASS_KEY = "rvu-bio-pass";
  var BIO_PIN_KEY = "rvu-bio-pin";
  ...
  function hasBiometrics() {
    return localStorage.getItem(BIO_ENABLED_KEY) === "true" && !!localStorage.getItem(BIO_CRED_KEY);
  }

  function enableBiometrics(pin) {
    var passphrase = u8toB64(crypto.getRandomValues(new Uint8Array(32)));
    return aesEncrypt(passphrase, pin).then(function(encPin) {
      var userId = crypto.getRandomValues(new Uint8Array(16));
      return navigator.credentials.create({
        publicKey: {
          challenge: crypto.getRandomValues(new Uint8Array(32)),
          rp: { id: location.hostname, name: "RVU Tracker" },
          user: { id: userId, name: "rvu-user", displayName: "RVU User" },
          pubKeyCredParams: [
        ...
        localStorage.setItem(BIO_CRED_KEY, u8toB64(new Uint8Array(cred.rawId)));
        localStorage.setItem(BIO_PASS_KEY, passphrase);
        localStorage.setItem(BIO_PIN_KEY, JSON.stringify(encPin));
        return true;
      });
    });
  }
  ...
  function disableBiometrics() {
    ...
    localStorage.removeItem(BIO_PASS_KEY);
    localStorage.removeItem(BIO_PIN_KEY);
  }

  function authenticateWithBiometrics() {
    if (!hasBiometrics()) return Promise.reject(new Error("Not set up"));
    var credIdB64 = localStorage.getItem(BIO_CRED_KEY);
    var credIdRaw = atob(credIdB64);
    var credIdBuf = new Uint8Array(credIdRaw.length);
    for (var i = 0; i < credIdRaw.length; i++) credIdBuf[i] = credIdRaw.charCodeAt(i);
    return navigator.credentials.get({
      ...
      var passphrase = localStorage.getItem(BIO_PASS_KEY);
      var encPin = JSON.parse(localStorage.getItem(BIO_PIN_KEY));
      return aesDecrypt(passphrase, encPin);
    });
  }
  ...
  function showPinScreen(mode) {
    ...
    if (mode === "locked") {
      ...
      if (hasBiometrics()) {
        ...
        bioSection.style.cssText = "margin-top:16px;text-align:center;";
        var bioBtn = document.createElement("button");
        bioBtn.id = "bio-unlock-btn";
        bioBtn.innerHTML = "&#x1F9EC; Use Biometrics";
        bioBtn.style.cssText = "padding:12px 24px;border-radius:12px;border:1px solid rgba(16,185,129,0.4);background:rgba(16,185,129,0.08);color:#10b981;font-size:14px;font-weight:600;cursor:pointer;font-family:inherit;";
        bioBtn.onclick = function() { tryBioUnlock(); };
        bioSection.appendChild(bioBtn);
        extra2.insertBefore(bioSection, forgotBtn);
        ...
        setTimeout(function() { tryBioUnlock(); }, 600);
      }
    }
